...
# Factor de conversión de píxeles a cm (ajusta este valor según la configuración de tu cámara y distancia)
PIXEL_TO_CM_CONVERSION = 0.026  # Ejemplo: 1 píxel ≈ 0.026 cm
...
def text(img, text, x, y):
    ...
    # Creamos un rectángulo negro con el tamaño apropiado
...
def dibujar_objetos(img, objects, umbral=0.5):
    ...
    for i in range(objects.shape[2]):
        ...
        # Extraemos sus coordenadas y las normalizamos a píxeles
        x = int(objects[0, 0, i, 3] * colum)
        y = int(objects[0, 0, i, 4] * filas)
        w = int(objects[0, 0, i, 5] * colum - x)
        h = int(objects[0, 0, i, 6] * filas - y)

        # Convertimos el tamaño de píxeles a centímetros
        width_cm = w * PIXEL_TO_CM_CONVERSION
        height_cm = h * PIXEL_TO_CM_CONVERSION

        # Revisamos si superamos el umbral
        if puntaje > umbral:
            # Mostramos la clase, dimensiones en píxeles y en cm
            label_text = "{} (W:{}px, H:{}px | W:{:.2f}cm, H:{:.2f}cm)".format(
                labels[clase], w, h, width_cm, height_cm
            )
            text(img, label_text, x, y)
...
while True:
    ...
    # Mostramos las detecciones con dimensiones en píxeles y cm
